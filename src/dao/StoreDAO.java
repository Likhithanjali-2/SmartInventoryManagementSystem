...
public class StoreDAO implements StoreDaoInterface {
	...
	public void displayProductsInStore() throws Exception {
		...
		while(rs.next()) {
			System.out.printf("%-6d %15s %20s %15d %15d %n",rs.getInt("productid"),rs.getString("category"),rs.getString("productname"),rs.getInt("quantity"),rs.getInt("price"));
		}
		System.out.println("__________________________________________________________________________________");
	}
	...
	public void displayCustomerPurchases() throws Exception {
		...
		while(rs.next()) {
			System.out.printf("%-6d %10s %15s %20s %15d %15d %n",rs.getInt("id"),rs.getString("name"),rs.getString("category"),rs.getString("productname"),rs.getInt("quantity"),rs.getInt("price")*rs.getInt("quantity"));
		}
		System.out.println("______________________________________________________________________________________________");
	}
